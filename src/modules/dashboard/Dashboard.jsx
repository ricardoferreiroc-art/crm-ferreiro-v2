...
import { AlertTriangle, Coins, Image, FileText, Bell, ArrowRight, Plus } from 'lucide-react'
...
export default function Dashboard() {
  ...
  async function cargar() {
    ...
    try {
      ...
      const [{ data: trabajos }, { data: segundo }, { data: leads }, { data: alertas }, { data: config }] = await Promise.all([
        supabase.from('v2_trabajos').select('id,tipo,titulo,fecha,hora_inicio,lugar,estado,precio_total,cobrado,estado_entrega,deleted_at')
          .eq('user_id', uid).is('deleted_at', null)
          .gte('fecha', `${anio}-01-01`).order('fecha', { ascending: true }),
        supabase.from('v2_segundo_fotografo').select('id,novios,fecha,hora,lugar,honorarios,cobrado,estado,deleted_at')
          .eq('user_id', uid).is('deleted_at', null)
          .gte('fecha', `${anio}-01-01`).order('fecha', { ascending: true }),
        supabase.from('v2_leads').select('id,nombre,tipo_trabajo,fecha_evento,estado,precio_acordado,created_at,deleted_at')
          .eq('user_id', uid).is('deleted_at', null)
          .not('estado', 'in', '(Confirmado,Descartado)')
          .order('created_at', { ascending: false }).limit(6),
        supabase.from('v2_alertas').select('id,titulo,descripcion,tipo,prioridad,fecha_alerta,completada,trabajo_id,lead_id')
          .eq('user_id', uid).eq('completada', false)
          .order('fecha_alerta', { ascending: true }).limit(8),
      ...
      const proximos = [
        ...
        ...(segundo||[]).filter(s=>s.fecha>=hoy).slice(0,3).map(s=>({id:s.id,titulo:s.novios||'—',tipo:'2º Fotógrafo',fecha:s.fecha,hora_inicio:s.hora,lugar:s.lugar,cobrado:s.cobrado,precio_total:s.honorarios,estado:s.estado,_k:'segundo'})),
      ...
      setD({
        kpis: { facturado, cobrado, bodas: (trabajos||[]).filter(t=>['Boda','Elopement'].includes(t.tipo)).length, leads: (leads||[]).length, objetivo: config?.objetivo_anual||80000 },
        proximos, leads: leads||[], alertas: alertas||[], cobros,
      })
  ...
  const PRIO_COLOR = { critica:'bg-red-100 text-red-700', alta:'bg-amber-100 text-amber-700', normal:'bg-brand/10 text-brand', baja:'bg-cream-dark text-ink-3' }

  if (loading) return <div className="flex items-center justify-center h-64 text-ink-3 text-sm">Cargando…</div>

  return (
    <div className="grid grid-cols-[1fr_290px] min-h-[calc(100vh-54px)]">
      <div className="p-7 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-4 gap-2.5">
          {[
            ...
            { l:'Cobrado', v:fmtEur(d.kpis.cobrado), s:`${fmtEur(d.kpis.facturado-d.kpis.cobrado)} pendiente` },
            { l:'Bodas confirmadas', v:d.kpis.bodas, s:'en '+anio },
            { l:'Leads activos', v:d.kpis.leads, s:'en pipeline' },
          ].map(k=>(
            <div key={k.l} className="card p-4">
              <p className="text-[10.5px] text-ink-3 mb-2">{k.l}</p>
              <p className={`font-serif text-[28px] leading-none ${k.hi?'text-brand':'text-ink'}`}>{k.v}</p>
              <p className="text-[11px] text-ink-3 mt-1.5">{k.s}</p>
            </div>
          ))}
        </div>
              ...
              : d.proximos.map((t,i)=>{
                ...
                return (
                  <Link key={t.id+(t._k||'')} to={t._k==='segundo'?`/trabajos/segundo/${t.id}`:`/trabajos/${t.id}`}
                    className={`flex items-center gap-3 px-4 py-2.5 hover:bg-brand/[.03] transition-colors ${i<d.proximos.length-1?'border-b border-cream-dark':''}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-medium flex-shrink-0 ${avatarColor(t.tipo)}`}>{iniciales(t.titulo)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] text-ink truncate">{t.titulo}</p>
                      <p className="text-[10.5px] text-ink-3">{fmtFecha(t.fecha)}{t.hora_inicio?` · ${t.hora_inicio}`:''}</p>
                    </div>
                    {t.lugar&&<span className="text-[10.5px] text-ink-3 text-right pr-2 flex-1 truncate">{t.lugar}</span>}
                    <span className={`text-[10px] px-2.5 py-0.5 rounded-full flex-shrink-0 ${tipoBadge(t.tipo)}`}>{t.tipo}</span>
                    <span className={`text-[10.5px] min-w-[28px] text-right flex-shrink-0 font-medium ${dias!==null&&dias<=3?'text-red-600':'text-ink-3'}`}>{dias!==null?`${dias}d`:'—'}</span>
                  </Link>
                )
              })
            }
            ...
            : <div className="card overflow-hidden">
                {d.leads.map((l,i)=>(
                  <Link key={l.id} to="/leads"
                    className={`flex items-center gap-3 px-4 py-2.5 hover:bg-brand/[.03] transition-colors ${i<d.leads.length-1?'border-b border-cream-dark':''}`}>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-medium flex-shrink-0 ${avatarColor(l.tipo_trabajo)}`}>{iniciales(l.nombre)}</div>
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ml-2 flex-shrink-0 ${
                      ['Contrato enviado','Firmado'].includes(l.estado)?'bg-brand/10 text-brand':'bg-cream-dark text-ink-3'}`}>{l.estado}</span>
                    {l.precio_acordado&&<span className="font-serif text-[13px] text-brand ml-2 flex-shrink-0">{fmtEur(l.precio_acordado)}</span>}
                  </Link>
                ))}
              </div>
          }
      ...
      <div className="border-l border-brand/[.08] p-5 flex flex-col gap-4">
        ...
        <div>
          <div className="flex items-center mb-2.5">
            ...
            {d.alertas.filter(a=>a.prioridad==='critica').length>0&&(
              <span className="ml-2 bg-red-600 text-white text-[9px] font-medium px-2 py-0.5 rounded-full">
                {d.alertas.filter(a=>a.prioridad==='critica').length} críticas
              </span>
            )}
                ...
                {d.alertas.map((a,i)=>(
                  <div key={a.id} className={`flex gap-2.5 py-2 ${i<d.alertas.length-1?'border-b border-cream-dark':''}`}>
                    <div className={`w-[26px] h-[26px] rounded-[6px] flex items-center justify-center flex-shrink-0 mt-0.5
                      ${PRIO_COLOR[a.prioridad]||PRIO_COLOR.normal}`}>
                      {ICONO[a.tipo]||<Bell size={12}/>}
                    </div>
                    <div>
