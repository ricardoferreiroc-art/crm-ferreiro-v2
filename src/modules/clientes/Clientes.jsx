...
import { Plus, Search, Mail, Phone, X, Save, Edit2, Trash2, Calendar, MapPin } from 'lucide-react'
...
export default function Clientes() {
  ...
  async function cargar() {
    ...
    const { data } = await supabase.from('v2_clientes')
      .select('*').eq('user_id', user.id).is('deleted_at', null).order('nombre')
    setClientes(data || [])
    setLoading(false)
  }
  ...
  async function cargarTrabajos(clienteId) {
    const { data } = await supabase.from('v2_trabajos')
      ...
      .eq('cliente_id', clienteId).is('deleted_at', null).order('fecha', { ascending: false })
  ...
  async function eliminar(id) {
    ...
    setClientes(c=>c.filter(x=>x.id!==id)); setSelected(null)
  }
  ...
  return (
    <div className="flex h-[calc(100vh-54px)]">
      ...
      {/* Lista — más ancha */}
      <div className="w-96 border-r border-brand/[.08] flex flex-col flex-shrink-0">
        <div className="p-4 border-b border-cream-dark">
          ...
          <div className="relative">
            ...
            <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} className="input pl-8 text-sm py-1.5 w-full" placeholder="Buscar por nombre, email, ciudad…"/>
          </div>
        </div>
          ...
          {loading ? <p className="text-sm text-ink-3 p-4 text-center">Cargando…</p>
            ...
            : filtrados.map(c=>(
              <button key={c.id} onClick={()=>setSelected(c)}
                className={`w-full text-left px-4 py-3.5 border-b border-cream-dark hover:bg-brand/[.03] transition-colors
                  ${selected?.id===c.id?'bg-brand/[.06] border-l-2 border-l-brand':''}`}>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-brand/10 text-brand flex items-center justify-center text-[11px] font-semibold flex-shrink-0">
                    {iniciales(`${c.nombre} ${c.apellidos||''}`)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-ink">{c.nombre} {c.apellidos}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      {c.email&&<span className="text-[10.5px] text-ink-3 truncate">{c.email}</span>}
                      {c.ciudad&&<span className="text-[10.5px] text-ink-3">· {c.ciudad}</span>}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      {c.telefono&&<span className="text-[10.5px] text-ink-3">{c.telefono}</span>}
                      {c.idioma&&c.idioma!=='es'&&<span className="text-[9px] bg-brand/10 text-brand px-1.5 py-0.5 rounded">{c.idioma.toUpperCase()}</span>}
                    </div>
          ...
          <div className="p-7 max-w-2xl">
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-brand text-white flex items-center justify-center text-base font-semibold">
                  {iniciales(`${selected.nombre} ${selected.apellidos||''}`)}
                </div>
                <div>
                  <h2 className="text-xl font-medium text-ink">{selected.nombre} {selected.apellidos}</h2>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    {selected.ciudad&&<span className="flex items-center gap-1 text-xs text-ink-3"><MapPin size={11}/>{selected.ciudad}{selected.provincia?`, ${selected.provincia}`:''}</span>}
            ...
            <div className="card p-5 mb-5">
              <p className="text-xs font-medium text-ink-2 uppercase tracking-wide mb-4">Datos de contacto</p>
              {editando?(
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="label">Provincia</label><input value={form.provincia||''} onChange={e=>set('provincia',e.target.value)} className="input text-sm"/></div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {selected.email&&<div className="flex items-center gap-2.5"><Mail size={14} className="text-ink-3 flex-shrink-0"/><a href={`mailto:${selected.email}`} className="text-sm text-brand hover:underline">{selected.email}</a></div>}
                  {selected.telefono&&<div className="flex items-center gap-2.5"><Phone size={14} className="text-ink-3 flex-shrink-0"/><a href={`tel:${selected.telefono}`} className="text-sm text-ink hover:text-brand">{selected.telefono}</a></div>}
                  {selected.pais&&selected.pais!=='España'&&<div><p className="text-xs text-ink-3">País</p><p className="text-sm text-ink">{selected.pais}</p></div>}
                  {selected.notas&&<div className="col-span-2 bg-cream rounded-lg p-3 text-sm text-ink-2">{selected.notas}</div>}
                </div>
              )}
            </div>
            ...
            <div className="card overflow-hidden">
              <div className="px-5 py-3 border-b border-cream-dark flex items-center justify-between">
                <p className="text-xs font-medium text-ink-2 uppercase tracking-wide">Historial de trabajos</p>
                <span className="text-xs text-ink-3">{trabajosCliente.length} trabajos</span>
              </div>
              {trabajosCliente.length===0
                ? <p className="text-sm text-ink-3 p-5 text-center">Sin trabajos vinculados</p>
                : trabajosCliente.map((t,i)=>(
                  <div key={t.id} className={`flex items-center gap-3 px-5 py-3.5 ${i<trabajosCliente.length-1?'border-b border-cream-dark':''}`}>
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${t.estado_entrega==='Entregado'?'bg-emerald-100 text-emerald-700':'bg-cream-dark text-ink-3'}`}>{t.estado_entrega||'Pendiente'}</span>
                    <span className="font-serif text-sm text-brand">{fmtEur(t.precio_total)}</span>
                  </div>
                ))
              }
            </div>
